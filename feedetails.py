...
class Fee:
    def __init__(self, root):
        ...
        lblimg = Label(self.root, text="Member Fee Details",
        ...
        labelframeright.place(x=600, y=70, width=390, height=400)
        ...
        imgfoot = Image.open(r"E:\Python\Projects\project-3-gym\img\6.jpg")
        imgfoot = imgfoot.resize((300, 227), Image.ANTIALIAS)
        self.photoimgfoot = ImageTk.PhotoImage(imgfoot)

        lblimgfoot = Label(
            self.root, image=self.photoimgfoot)
        lblimgfoot.place(x=0, y=70, width=300, height=227)
    ...
    def fetch_data(self):
        conn = mysql.connector.connect(
            host="localhost", username="root", password="gaurav4u7", database="gym"
        )
        my_cursor = conn.cursor()
    ...
    def totalfee(self):
        inDate = self.feedate.get()
        inDate = datetime.strptime(inDate, "%d/%m/%Y")

        if (self.membership.get() == "Basic"):
            q1 = float(700)
            tax = "Rs" + str("%.2f" % ((q1)*0.1))
            stotal = "Rs" + str("%.2f" % ((q1)))
            Ttotal = "Rs" + str("%.2f" % (q1+((q1)*0.1)))

            self.paidtax.set(tax)
            self.fee.set(stotal)
            self.total.set(Ttotal)

        elif (self.membership.get() == "Special"):
            q1 = float(1000)
            tax = "Rs" + str("%.2f" % ((q1)*0.1))
            stotal = "Rs" + str("%.2f" % ((q1)))
            Ttotal = "Rs" + str("%.2f" % (q1+((q1)*0.1)))
        ...
        elif (self.membership.get() == "Premium"):
            q1 = float(1300)
            tax = "Rs" + str("%.2f" % ((q1)*0.1))
            stotal = "Rs" + str("%.2f" % ((q1)))
            Ttotal = "Rs" + str("%.2f" % (q1+((q1)*0.1)))
